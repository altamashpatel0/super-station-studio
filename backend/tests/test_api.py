...
def test_playable_mp3_with_bad_tags_appears_in_library_via_api(
    client, music_folder, make_mp3_with_unreadable_tags
):
    """
    End-to-end regression test for the reported bug, through the real
    HTTP API a UI would call: a playable MP3 with Mutagen-unreadable
    tags must actually show up in `GET /api/library/songs` after a
    scan, with fallback metadata and a real duration - not be silently
    dropped.
    """
    ...
    assert job["status"] == "completed"
    assert job["errors"] == 0
    assert job["added"] == 4  # 3 normal tracks + the problem file

    songs = client.get("/api/library/songs").json()
    assert len(songs) == 4

    recovered = next(s for s in songs if s["file_name"] == "song.mp3")
    assert recovered["title"] == "song"
    assert recovered["artist"] == "Unknown Artist"
    assert recovered["album"] == "Unknown Album"
    assert recovered["genre"] == "Unknown"
    assert recovered["duration"] > 0
    assert recovered["enabled"] is True

    # Also reachable via search/filter, exactly as any other track.
    found_by_id = client.get(f"/api/library/songs/{recovered['id']}").json()
    assert found_by_id["title"] == "song"
