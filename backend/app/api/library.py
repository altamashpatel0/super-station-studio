...
def get_song_artwork(song_id: int, db: Session = Depends(get_db)):
    """Return embedded album artwork for a library track, when available."""
    song = SongRepository(db).get_by_id(song_id)
    if song is None:
        raise HTTPException(status_code=404, detail=f"No song with id {song_id}.")
    ...
    try:
        ...
        if pictures:
            picture = pictures[0]
            mime = getattr(picture, "mime", "image/jpeg") or "image/jpeg"
            return Response(content=picture.data, media_type=mime, headers={"Cache-Control": "public, max-age=86400"})

        # Some containers expose a generic picture list rather than APIC frames.
